...
class PlayerGuess{
    public static int PlayerInput(String[][][]playerBoard,String[][][]compBoard,ArrayList<String> playerShips,ArrayList<String> aiShips,ArrayList<Move> moveHistAI,int hit){
        ...
        while(!cordsSet){
            ...
            while(!gotYCoord){
                selectScreen(playerBoard,compBoard);
                System.out.print("\nYou can start your guess over by typing CANCEL");
                System.out.print("\n("+xCoord+",");
                yCoord= getCoord();

                if(yCoord>0){
                    gotYCoord=true;
                    if((compBoard[0][Main.gTOaY(yCoord)][Main.gTOaX(xCoord)]==null)||!compBoard[0][Main.gTOaY(yCoord)][Main.gTOaX(xCoord)].equals("yes")){
                        cordsSet=true;
                        done=true;
                        gotYCoord=true;
                    }
    ...
    public static int guessResults(String[][][]adjBoard,ArrayList<String> shipList,ArrayList<Move> moveHist,int x,int y){
        ...
        if(boardPlace==null){
            ...
            if(shipContain(adjBoard,boardPlace)){
                ...
                else if(boardPlace.equals("e")){
                    SunkShip="\uD83C\uDD98";
                }
                for(int z = 0;z<10;z++){
                    for(int c = 0; c<10;c++){
                        try{
                            if(adjBoard[2][z][c].equals(boardPlace)){
                                adjBoard[1][z][c] = (SunkShip+" \u001B[0m ");
                            }
                        }
                        catch(Exception e){}
                    }
